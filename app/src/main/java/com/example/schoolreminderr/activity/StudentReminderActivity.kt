...
import com.example.schoolreminderr.adapter.StudentReminderAdapter
...
class StudentReminderActivity : AppCompatActivity() {
    ...
    private lateinit var adapter: StudentReminderAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rvReminder      = findViewById(R.id.rvReminderStudent)
        ...
        adapter = StudentReminderAdapter(filteredReminders)
            ...
            .enqueue(object : Callback<ReminderResponse> {
                override fun onResponse(call: Call<ReminderResponse>, response: Response<ReminderResponse>) {
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        Toast.makeText(this@StudentReminderActivity, "Gagal load reminder", Toast.LENGTH_SHORT).show()
                    }
                }
                override fun onFailure(call: Call<ReminderResponse>, t: Throwable) {
                    Toast.makeText(this@StudentReminderActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
    }
    ...
    private fun setupNavbar() {
        ...
        findViewById<ImageButton>(R.id.btnNotif).setOnClickListener { /* Tetap Kosong */ }
    ...
    private fun setNavActive(activeId: Int) {
        // Tambahkan btnProfile ke dalam list agar ikut ter-reset warnanya
        val navIds = listOf(R.id.btnHome, R.id.btnClass, R.id.btnNotif, R.id.btnProfile)

        navIds.forEach { id ->
            val btn = findViewById<ImageButton>(id)
            btn?.apply { // Gunakan ?. agar tidak crash jika tombol tidak ditemukan
